...
let get_common_node_patterns gss env =
  ...
  in 
    ...
    let count node_p =
      try (
	let cnt = Diff.TT.find common_ht node_p in
	  Diff.TT.replace common_ht node_p (cnt + 1)
      )
      with Not_found -> (
	Diff.TT.replace common_ht node_p 1;
      ) in
	...
	abs_terms_lists
	+> List.iter (function abs_terms ->
			abs_terms +> List.iter
			  (function a_term -> 
			     count a_term
			  )
		     );
	Diff.TT.fold (fun node_p cnt count_list ->
			(node_p, cnt) :: count_list
		     ) common_ht []
      )
...
let construct_spatches patterns is_freq =
  ...
  in 
    ...
    in
       ...
       *)
      print_endline "[Main] making init_spatches";
...
let find_common_patterns () = (
  ...
  let term_pairs = List.rev (
	...
	let node_cnt_list = 
	  get_common_node_patterns flat_gs [] 
	  +> List.filter (function (_, cnt) -> cnt >= !threshold)
	  +> List.sort (fun (_, cnt1) (_, cnt2) -> cnt2 - cnt1)	    
	in
	  print_endline "[Main] patterns found:";
	  node_cnt_list 
	  +> List.iter (function (node_p, cnt) -> 
			  if !verbose
			  then 
			    print_endline (string_of_int cnt ^ "\t" ^ Diff.verbose_string_of_gtree node_p)
			  else 
			    print_endline (string_of_int cnt ^ "\t" ^ Diff.string_of_gtree' node_p)
		       )
)
